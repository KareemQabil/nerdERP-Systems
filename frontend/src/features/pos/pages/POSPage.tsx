...
import { selectLanguage, selectTheme } from '@/features/settings/slices/settingsSlice';
import {
    addItem,
    clearCart,
    setCustomer,
...
import { verifyPin, selectRequiresManagerAuth } from '@/features/auth/slices/authSlice';

// Keep Zustand stores during migration for non-migrated functionality
...
export default function POSPage() {
    ...
    const cartNotes = useAppSelector(selectNotes);

    // Zustand stores (kept during migration)
    ...
    const handleAddProduct = (productId: string) => {
        ...
        if (product) {
            dispatch(addItem({ product: mapProductToProductInfo(product) }));
    ...
    const handleRecallOrder = (order: HeldOrder) => {
        ...
        order.items.forEach(item => dispatch(addItem({ product: item.product })));
    ...
    // Manager authorization check using Redux selector
    const requiresAuth = useAppSelector(selectRequiresManagerAuth('APPLY_DISCOUNT'));

    const handlePinAuthorize = async (pin: string, reason?: string): Promise<boolean> => {
        if (!pendingPinRequest) return false;
        const result = await dispatch(verifyPin({ pin, action: pendingPinRequest.action, reason })).unwrap();
