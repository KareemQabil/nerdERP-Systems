...
import type { KitchenStatus, VoidReason, PinAuthorizationRequest } from '@/features/pos/types';
...
export function VoidItemButton({
    ...
    const getStatusMessage = () => {
        ...
        switch (kitchenStatus) {
            case 'PENDING':
                return null; // Can remove freely
            case 'FIRED':
                return language === 'ar' ? 'تم إرسال الطلب للمطبخ' : 'Sent to kitchen';
            case 'PREPARING':
                return language === 'ar' ? 'قيد التحضير' : 'Being prepared';
            case 'READY':
                return language === 'ar' ? 'جاهز للتقديم' : 'Ready to serve';
            case 'SERVED':
                return language === 'ar' ? 'تم التقديم' : 'Already served';
            default:
                return null;
        }
    };
