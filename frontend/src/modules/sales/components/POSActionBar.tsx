...
export function POSActionBar({
    ...
    const actions: ActionButton[] = [
        ...
        { key: 'endShift', icon: LogOut, labelKey: 'actions.endShift', shortcut: 'F9', onClick: onEndShift },
        // Only show EOD button for managers
        ...(isManager && onEOD ? [{ key: 'eod', icon: FileText, labelKey: 'actions.eod', shortcut: 'F10', onClick: onEOD }] : []),
    ];
            ...
            >
                ...
                {pendingSyncCount > 0 && (
                    <motion.div
                        ...
                        className="ms-auto flex items-center gap-2 px-2 py-1 rounded-lg bg-red-500/20 border border-red-500/40 cursor-pointer hover:bg-red-500/30"
                        title={isSyncing ? 'Syncing...' : 'Click to sync now'}
                        onClick={(e) => {
                            e.stopPropagation();
                            if (!isSyncing && onSync) onSync();
                        }}
                    >
                        {isSyncing ? (
                            <motion.div
                                ...
                                className="w-2 h-2 border-2 border-red-500 border-t-transparent rounded-full"
                            />
                        ) : (
                            <div className="w-2 h-2 rounded-full bg-red-500 animate-pulse" />
                        )}
                        <span className="text-[10px] font-bold text-red-500 uppercase tracking-tighter">
                            {pendingSyncCount} {t('offline.pending', 'Pending')}
                        </span>
                    </motion.div>
                )}
            </motion.button>
